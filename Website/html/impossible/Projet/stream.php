...
if (is_dir($nomRepertoire)) {
   ...
   if ($dossier) {
       while ($Fichier = readdir($dossier)) {
           if ($Fichier != "." && $Fichier != ".." && exif_imagetype($nomRepertoire."/".$Fichier) === IMAGETYPE_JPEG) {
               $h_vign = "120";
               $taille = getimagesize($nomRepertoire."/".$Fichier);
               $reduc  = floor(($h_vign * 100) / ($taille[1]));
               $l_vign = floor(($taille[0] * $reduc) / 100);
      
               echo '<a target="_blank" href="', htmlspecialchars($urlphoto.'/'. $Fichier), '">';
               echo '<img src="', htmlspecialchars($urlphoto.'/'. $Fichier), '" ';
